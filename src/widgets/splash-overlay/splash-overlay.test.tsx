...
describe('SplashOverlay', () => {
  ...
  it('첫 방문 시 오버레이를 표시한다', () => {
    ...
    expect(screen.getByText('MBTI 그룹 케미 시뮬레이터')).toBeInTheDocument();
  });

  it('2초 후 페이드아웃을 시작한다', () => {
    render(<SplashOverlay />);

    const overlay = screen.getByText('MINGLE').closest('div[class*="fixed"]')!;
  ...
  it('트랜지션 완료 후 DOM에서 제거되고 sessionStorage에 기록한다', () => {
    ...
    expect(screen.queryByText('MINGLE')).not.toBeInTheDocument();
